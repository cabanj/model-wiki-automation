...
- `gen.py` writes `dist/index.html`, `dist/comparisons-free-models-ranking.html`, and `dist/comparisons-router-changelog.html`.
...
`run.sh` requires Linux, `AA_API_KEY`, `sudo`, `/var/www/model-wiki`, and an HTTP server on `127.0.0.1:8080`. It deploys only the four named HTML files. `cron-model-wiki.sh` performs `git pull --ff-only`, sources `/etc/model-wiki.env`, and expects an external cron schedule; it does not install a schedule or redirect logs despite its comment.
...
- Router changelog input is external at `/opt/hermes-router/data/changelog.json`; missing or invalid data renders an empty changelog.
